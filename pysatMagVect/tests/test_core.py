...
#                                   
    ...
    def test_unit_vector_component_fieldline_sensitivity_plots(self):
        import matplotlib.pyplot as plt
        # from mpl_toolkits.mplot3d import Axes3D
        import os
        latitudes = np.zeros(100)*35.
        longitudes = np.arange(361.)
        altitudes = 550. + 0 * longitudes
        date = pysat.datetime(2019,1,1)
        
        init = pymv.geographic_to_ecef(latitudes, longitudes, altitudes)
        # for each longitude and latitude, do a field line trace and look at vector
        # sensititivty.
        
        p_lats, p_longs, p_alts = pymv.full_field_line(init, date, 100.)
